...
function bundleButtonText(status) {
	return status.core_installed && status.font_installed
		? _('Update core and font')
		: _('Download core and font');
}
		...
		s.render = function() {
			poll.add(function() {
				return Promise.all([ callStatus(), callCoreUpdateStatus() ]).then(function(results) {
					...
					updateButtonNode.disabled = !!update.running;
					updateButtonNode.textContent = update.running
						? _('Downloading core and font...')
						: bundleButtonText(current);
					if (update.log) {
			...
			function updateButton() {
				updateButtonNode = E('button', {
					'class': 'cbi-button cbi-button-action',
					disabled: status.update_running ? '' : null,
					click: function(ev) {
						ev.preventDefault();
						ev.currentTarget.disabled = true;
						ev.currentTarget.textContent = _('Downloading core and font...');
						return callCoreUpdateStart('all', false).then(function(result) {
							if (!result.success)
								throw new Error(result.error || _('Unable to start the download'));
							ui.addNotification(null, E('p', {}, _('Download started')));
						}).catch(function(err) {
							ev.currentTarget.disabled = false;
							ev.currentTarget.textContent = bundleButtonText(status);
							ui.addNotification(null, E('p', {}, err.message), 'error');
						});
					}
				}, bundleButtonText(status));
				return updateButtonNode;
			}
